...
function Purchase({ addToCart }) { // <-- accept addToCart as prop
  const navigate = useNavigate();
  const [items, setItems] = useState([]);
  const [itemsInCart, setItemsInCart] = useState([]);

  const handleItemsInCart = (property, e) => {
    if (e && e.stopPropagation) e.stopPropagation(); // prevent opening product page
    if (typeof addToCart === "function") addToCart(property);

    // add this item object to local itemsInCart state (avoid duplicates by id)
    if (!itemsInCart.find((i) => i.id === property.id)) {
      setItemsInCart((prev) => [...prev, property]);
    }

    // navigate to Cart page so user sees the cart after adding
    navigate("/Cart");
  };
  ...
  return (
    <div className="bg-gray-500 text-black mt-20 mb-20 p-1 ">
      ...
      <div>
        <span className="bg-blue-950 text-white p-0.5 rounded">Official store</span>{" "}
        <span className="bg-black text-white p-0.5 rounded">Black Friday deal</span>
        <br />
        {product.name}
        <br />
        {product.family} <br />
      ...
      <div>
        ...
        <div className="bg-white text-blue-800 text-left justify-center my-2">
          <div>
            <span className="text-amber-500 text-2xl text-center">
          ...
          <div>
            ...
            Free delivery in Accra and Kumasi for orders above $15. Pick-up stations only
          </div>
          <br />
          <div>
            <span className="text-amber-500 text-2xl text-center">
            ...
            Need advice or assistance to place order? contact us at +233 597788861
          </div>
        </div>
      </div>
      ...
      <div className="bg-white text-2xl flex gap-5">
        ...
        <a href="tel:+233597788861">
          <button className=" text-amber-500 outline m-1 p-1 rounded">
            <ion-icon name="call-outline"></ion-icon>
          </button>
        </a>

        {/* wire the button to call handleItemsInCart and navigate to Cart */}
        <button
          onClick={(e) => handleItemsInCart(product, e)}
          className=" bg-amber-500 text-white flex items-center m-1 p-1 rounded"
        >
